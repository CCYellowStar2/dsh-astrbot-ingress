  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'assistant/message') {
      ...
      if (cleanText) {
        ...
        if (turn.pendingText) sseWrite(turn.res, 'text', { text: turn.pendingText })
        for (let i = 0; i < chunks.length; i++) {
          if (i === chunks.length - 1) turn.pendingText = chunks[i]
          else sseWrite(turn.res, 'text', { text: chunks[i] })
        }
      }
