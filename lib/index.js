 ...
 */
const CAPABILITIES = ['url-inbound-probe', 'file-token', 'withdraw-web-card']
...
export function apply(ctx, config = {}) {
  ...
  async function handleInbound(body, res) {
    ...
    const watchdog = setInterval(() => {
      ...
      sseWrite(record.res, 'status', { text: '这一轮没有新的输出（消息可能已在网页上撤回，或 DSH 那边没跑起来）。' })
      sseWrite(record.res, 'done', { sessionId })
      try { record.res.end() } catch { /* ignore */ }
      state.turns.delete(sessionId)
      stopHeartbeat(sessionId)
  ...
  ctx.effect(() => async () => {
    ...
    for (const [sessionId, turn] of state.turns) {
      try { sseWrite(turn.res, 'done', { sessionId, reason: 'ingress-reload' }) } catch { /* ignore */ }
      try { turn.res.end() } catch { /* ignore */ }
    }
    state.turns.clear()
    await clearBeacon()
