...
export function apply(ctx, config = {}) {
  ...
  function flushHeldText(turn) {
    if (!turn) return
    clearHeldTimer(turn)
    const text = turn.pendingText
    turn.pendingText = ''
    if (text && turn.res && !turn.res.writableEnded) sseWrite(turn.res, 'text', { text })
  }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (Number.isFinite(turnNo)) {
      if (staleTurns.get(sessionId)?.has(turnNo)) return
      if (turn.turnNo == null) turn.turnNo = turnNo
    ...
    if (event.type === 'assistant/message') {
      ...
      if (cleanText) {
        ...
        flushHeldText(turn) // 上一条助手消息压着的那段，先补发
  ...
  async function handleInbound(body, res) {
    ...
    if (existing?.res && !existing.res.writableEnded) {
      ...
      // 再记下它的轮次号——它迟到的事件会被丢掉。
      flushHeldText(existing)
      markStaleTurn(sessionId, existing.turnNo)
      sseWrite(existing.res, 'status', { text: '被新消息打断' })
      try { existing.res.end() } catch { /* ignore */ }
    }

    state.turns.set(sessionId, { umo, senderId, res, chatType, progress: resolveProgress(body?.progress) })
    res.on('close', () => {
