...
/**
 * 只探活：GET 到响应头（再读一小段确认真能取到数据就断开），**不落盘**。
 * 用来在真正传文件之前确认某个地址从这台机器能不能访问到。
 */
export async function probeUrl(url, options = {}) {
  const { timeoutMs = 5_000, fetchImpl = globalThis.fetch } = options
  if (!isHttpUrl(url)) return { ok: false, status: 0, error: '只支持 http/https 链接' }
  if (typeof fetchImpl !== 'function') return { ok: false, status: 0, error: '当前运行时没有 fetch' }
  const ac = new AbortController()
  const timer = setTimeout(() => ac.abort(new Error(`探测超时（${timeoutMs}ms）`)), timeoutMs)
  try {
    ...
    try {
      ...
      if (reader) {
        ...
        if (!done && value) bytes = value.length
        await reader.cancel().catch(() => {})
      }
    } catch { /* 读 body 失败不影响「地址可达」这个判断 */ }
    return { ok: Boolean(res.ok), status: res.status, contentType, bytes }
  } catch (err) {
