...
- **未绑定时 `/dsh follow` 会自动接上正在跑的会话**（用户要的「像在群里启动这个会话一样」）。
  以前未绑定直接 400，现在分三种情况：

  | 情况 | 行为 |
  |---|---|
  | 未绑定 + **恰好一个**会话在跑 | 用它，**并顺手把这个 QQ 会话绑上**（等同 `/dsh use` 过了） |
  | 未绑定 + **多个**在跑 | **不猜**，409 列出候选（标题 + 短 id），让人先 `/dsh use <序号>` |
  | 未绑定 + **没有**在跑的 | 400 说清「先 /dsh <任务> 起一个」 |

  **为什么多个时坚决不猜**：DSH 里可能同时跑着好几个会话，而「你要的是哪个」桥这边判断不了
  （网页当前打开的那个只是浏览器内存状态，ingress 读不到）。猜错会把**别人的输出**接到这个群，
  比报错更糟。

- 插件侧不再写死 400/409 的文案，改为**照读 ingress 给的 `error`** —— 否则会出现
  「实际是有多个在跑、却告诉用户没有在跑的回合」这种自相矛盾。

### 修掉一个自己写出来的判据错误

- `runningSessions()` 第一版把「`state.turns` 里有记录」也当成「在跑」，于是**回合刚结束、
  孤儿记录还没被看门狗收走**时候选数会虚高 —— 实测表现是：自动绑定成功一次之后，
  下一次 follow 被误判成「有 3 个在跑」而拒绝自动接上。现在判据收紧为
  **有活着的 sink，或 agent 状态确实是 running**。

### 验证

`follow_adopt_harness.mjs`：三个分支各断言一次（400 / 200+绑定落到 sessions.json / 409 且不绑定）
——全部通过。加上此前四个 harness（QQ 发起、网页发起、流掐断、崩溃复现）共 **5 个全绿**。
