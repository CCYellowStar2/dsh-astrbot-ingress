...
- **白名单**：管理员永远放行；群聊看 `allow_groups`，私聊看 `allow_users`（互不通用）。私聊里名单外的人不会收到拒绝提示，消息照常交给人格；群聊里名单外**明确在调桥**（发 `/dsh …`，或引用 DSH 的回复）会回一句说明。**纯数字 / `批准` / `取消` 这类「回答」不算** —— 它们是被特意接住的（免得被人格当闲聊答掉），名单外的人静默放行：本来也答不了（ingress 按发起人校验），不至于有人在群里随手打个 `1` 就被怼一句
