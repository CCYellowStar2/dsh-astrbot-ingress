...
- **发新版本：改 `metadata.yaml` 的 `version` 推镜像仓库，然后去发布页提交一次**（保守做法，别指望自动）。
  2026-09-12 的时间线：推 0.3.0（`5b2901b`，08:56:09Z）→ 提交记录 09:01:21Z → 09:03:14Z 发布。
  当时我判成「Cloud 自动跟版」，**但同一份文档后面的实测推翻了这个结论**：推 0.3.1（09:2xZ）后
  55 分钟内没有被自动扫到（后台任务盯着，0.3.1 一直没出现）。所以 09:01:21 那次提交要么是手动点的，
  要么是**整点定时扫描**（那个时刻正好是 `HH:01`）——两种都解释得通，但**都不该拿来做发版依据**。
  结论记牢：**推版本号 + 到发布页手动提交一次**；自动那部分当它不存在。
  手动 update 模式要选已有插件重新解析 GitHub，版本号必须大于已发布版本
  （前端有 `publish.versionInvalid` 校验）。
- **市场页面用的是发布时的快照**（2026-09-12 实测）：改完 README / CHANGELOG 再推仓库，
  页面上还是旧的；要让文案生效必须**发一版新的**（我们就是为这个发了 0.3.1）。
