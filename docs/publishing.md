...
- **发新版本：改 `metadata.yaml` 的 `version` 推镜像仓库，然后到发布页「解析自己的仓库 → 更新」**。
  2026-09-12 的时间线：推 0.3.0（`5b2901b`，08:56:09Z）→ 提交记录 09:01:21Z → 09:03:14Z 发布；
  推 0.3.1（09:2xZ）后 55 分钟没有任何自动动静，**是手动走「解析仓库 → 更新」提交的**：
  09:46:37Z 提交 → **09:47:34Z 发布（57 秒）**，VirusTotal 与 claude-code 都过。
  所以别指望自动跟版（云侧虽有「定时检查已发布插件」这类设置，但间隔不可预期）；
